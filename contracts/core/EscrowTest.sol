...
contract EscrowTest is AccessControlEnumerable {
    ...
    function createEscrowFromHotWallet(CreateEscrowFromHotWalletInput memory input) external payable {
    ...
    function _validateClaim(EscrowData storage escrow, string memory hash) internal view {
        require(escrow.info.isActive, "Escrow is not active");
        require(stringsEqual(escrow.info.hash, hash) , "Invalid hash");
    ...
    function getEscrowIdByHash(string memory hash) internal view returns (uint256) {
        for (uint256 i = 0; i < escrowCount; i++) {
            if (stringsEqual(escrows[i].info.hash, hash)) {
                return i;
            }
        }
